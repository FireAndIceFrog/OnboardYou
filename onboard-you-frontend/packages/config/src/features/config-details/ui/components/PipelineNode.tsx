import { Handle, Position, type NodeProps } from '@xyflow/react';
import { useTranslation } from 'react-i18next';
import { Box, Flex, Text } from '@chakra-ui/react';
import { useAppSelector } from '@/store';
import type { RootState } from '@/store';
import { businessLabel } from '@/shared/domain/types';
import { ImportIcon, CogIcon, ExportIcon, WrenchIcon } from '@/shared/ui';

/* ── Style lookup by category ──────────────────────────────── */

interface CategoryStyle {
  color: string;
  badgeBg: string;
  badgeColor: string;
  icon: React.ComponentType<{ size?: number | string }>;
  i18nKey: string;
}

const CATEGORY_STYLES: Record<string, CategoryStyle> = {
  ingestion: {
    color: 'secondary.500',
    badgeBg: 'secondary.50',
    badgeColor: 'secondary.700',
    icon: ImportIcon,
    i18nKey: 'configDetails.nodes.dataSource',
  },
  logic: {
    color: 'secondary.500',
    badgeBg: 'secondary.50',
    badgeColor: 'secondary.700',
    icon: CogIcon,
    i18nKey: 'configDetails.nodes.businessRule',
  },
  egress: {
    color: 'primary.500',
    badgeBg: 'primary.50',
    badgeColor: 'primary.500',
    icon: ExportIcon,
    i18nKey: 'configDetails.nodes.destination',
  },
};

const DEFAULT_STYLE: CategoryStyle = {
  color: 'tertiary.500',
  badgeBg: 'tertiary.50',
  badgeColor: 'tertiary.700',
  icon: WrenchIcon,
  i18nKey: 'configDetails.nodes.step',
};

/**
 * Unified React Flow node component.
 * Replaces the former IngestionNode, TransformationNode, and EgressNode
 * with a single data-driven component whose appearance is controlled
 * by the `category` field in the node's data.
 */
export function PipelineNode({ data, selected }: NodeProps) {
  const { t } = useTranslation();
  const category = (data.category as string) ?? 'logic';
  const style = CATEGORY_STYLES[category] ?? DEFAULT_STYLE;
  const friendly = businessLabel(data.actionType as string);
  const actionId = (data.actionId as string) ?? '';
  const Icon = style.icon;

  // Select only this action's error (primitive string|undefined) to avoid
  // re-renders when the validationErrors object reference changes.
  const error = useAppSelector(
    (state: RootState) => state.configDetails.validationErrors[actionId],
  );

  return (
    <Box
      bg={error ? 'red.50' : 'white'}
      border="2px solid"
      borderColor={error ? 'red.300' : selected ? 'secondary.500' : 'tertiary.200'}
      borderRadius="lg"
      borderLeft="4px solid"
      borderLeftColor={error ? 'red.500' : style.color}
      shadow={selected ? 'lg' : error ? 'md' : 'sm'}
      minW="180px"
      p="3"
      transition="all 0.15s ease"
      _hover={{ shadow: 'md', borderColor: error ? 'red.400' : 'secondary.400' }}
      cursor="pointer"
      outline={selected ? '2px solid' : 'none'}
      outlineColor={selected ? 'secondary.300' : 'transparent'}
      outlineOffset="2px"
      data-testid={`pipeline-node-${category}`}
    >
      <Flex align="center" gap="2" mb="1">
        <Box color={style.badgeColor} flexShrink={0}>
          <Icon size="1em" />
        </Box>
        <Text
          fontSize="xs"
          fontWeight="600"
          color="tertiary.500"
          textTransform="uppercase"
          letterSpacing="wide"
        >
          {t(style.i18nKey)}
        </Text>
      </Flex>
      <Text fontSize="sm" fontWeight="500" mb="2" color="primary.500">
        {data.label as string}
      </Text>
      <Box
        px="2"
        py="0.5"
        borderRadius="full"
        bg={style.badgeBg}
        color={style.badgeColor}
        fontSize="xs"
        fontWeight="500"
        display="inline-block"
      >
        {friendly}
      </Box>
      {error && (
        <Flex align="center" gap="1" mt="2">
          <Text fontSize="xs" color="red.600" lineClamp={2}>
            {error}
          </Text>
        </Flex>
      )}
      <Handle type="target" position={Position.Left} />
      <Handle type="source" position={Position.Right} />
    </Box>
  );
}
