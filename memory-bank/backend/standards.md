...
| `make smoke-test` | Sync env from tofu outputs + run Vitest smoke tests |
...
4. Route handlers in `lambdas/api/src/controllers/` (registered in `main.rs`)
5. Use `#[serde(rename_all = "camelCase")]` on all API models
6. **Request DTOs:** Use separate request structs (e.g. `ConfigRequest`, `SettingsRequest`) that omit server-controlled fields (`organizationId`, `customerCompanyId`, `lastEdited`). The controller fills those from JWT Claims + Path params.
7. Register new OpenAPI schemas in `main.rs` `ApiDoc` derive macro
...
To add a new path to the API Gateway:

1. Add an entry to the `routes` list in the `module "api"` block in `infra/main.tf`:
   ```hcl
   routes = [
     { path = "config",   methods = ["GET"] },    # exact path with specific methods
     { path = "config/*" },                        # wildcard proxy (ANY)
     { path = "auth/*",   auth = false },          # unauthenticated proxy
     { path = "settings", methods = ["GET", "PUT"] },
   ]
   ```
   - Paths ending with `/*` create a `{proxy+}` catch-all (ANY method)
   - Set `auth = false` for unauthenticated routes (e.g. login)
2. The API Gateway module (`infra/modules/api-gateway/`) automatically creates the resource, methods, integrations, and CORS OPTIONS pre-flight.
3. Add the Axum route in `lambdas/api/src/main.rs` `router()` function.

### "I need to add or update smoke tests"

Smoke tests live in `test/smoke-test/` (Vitest 3.2.4 + TypeScript).

1. Test files go in `src/services/{feature}.smoke.ts`
2. Use the shared `client` from `src/env.ts` (auto-login via `beforeAll`)
3. Model types in `src/models/`
4. Run: `make smoke-test` (syncs `.env` from tofu outputs, then runs vitest)
5. Environment: `.env` is auto-generated by `sync-env.sh` — never edit manually
6. **Important:** `lambda_http` 0.10 prepends the API Gateway stage prefix to paths. The env var `AWS_LAMBDA_HTTP_IGNORE_STAGE_IN_PATH=true` is set on the config-api Lambda to disable this.
7. **Important:** The `sync-env.sh` script single-quotes all values to prevent dotenv from truncating at `#` or expanding `$`.
